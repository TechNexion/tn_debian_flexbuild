...
tn_bluez:
	@[ $(DESTARCH) != arm64 -o $(SOCFAMILY) != IMX ] && exit || \
	 ...
	 install -m 0644 $(PKGDIR)/apps/networking/tn_bluez/recipes-connectivity/bluez5/files/serial-btattach@.timer $(DESTDIR)/usr/lib/systemd/system/ && \
	 install -m 0644 $(FBDIR)/src/apps/networking/tn_bluez/setup-serial-btattach.service $(DESTDIR)/usr/lib/systemd/system/  && \
	 install -m 0755 $(FBDIR)/src/apps/networking/tn_bluez/setup_serial_btattach.sh $(DESTDIR)/usr/sbin/  && \
	 ...
	 cd $(PKGDIR)/apps/networking/tn_bluez && \
	 if [ ! -d $(PKGDIR)/apps/networking/tn_bluez/bluez ];then \
	 ...
	 git am  \
		...
		$(PKGDIR)/apps/networking/tn_bluez/recipes-connectivity/bluez5/files/0003-hciattach-instead-of-strlcpy-with-strncpy-to-avoid-r.patch \
		$(PKGDIR)/apps/networking/tn_bluez/recipes-connectivity/bluez5/files/0004-Add-support-for-Tufello-1.1-SOC.patch \
		$(PKGDIR)/apps/networking/tn_bluez/recipes-connectivity/bluez5/files/0005-bluetooth-Add-support-for-multi-baud-rate.patch \
		$(PKGDIR)/apps/networking/tn_bluez/recipes-connectivity/bluez5/files/0001-hciattach_rome-do-not-override-module-MAC-address.patch \
	 ...
	 install -m 0755 $(PKGDIR)/apps/networking/tn_bluez/bluez/tools/btgatt-client $(DESTDIR)/usr/bin/ && \
